...
def estimate_translation(S, joints_2d, focal_length=5000., img_size=224.):
    """
    This function is borrowed from https://github.com/nkolot/SPIN/blob/master/utils/geometry.py
...
def rot6d_to_axis_angle(x):
    mat = rot6d_to_rotmat(x)
    pad_tensor = F.pad(mat, [0, 1])

    return rotation_matrix_to_axis_angle(pad_tensor)
...
def convert_to_6D_rot(pose):
    if len(pose.shape) == 2:
        # (B, 3) 
        pose = rodrigues(pose) # return [B, 3,3]
        
    # first two rows
    # xr_repr =  pose[:, :2, :].reshape(-1,6)
